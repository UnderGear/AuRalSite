// nexus PositionArray
// Jesse Allison. Yemin Oh 2012
//
// An array of 2D (x,y) positions - option-click to add/remove balls.	
//
...
function positionArray(canvas, ajax_command, ui_id) {
	...
	this.line_width = 3.;
	this.node_size = 15;
	this.default_text = "option-click to add/remove nodes";
	...
	function draw_node(slider2d_context) {
		with (slider2d_context) {
			...
			for(i=0; i<node_array.length; i++) {
				...
				lineCap = "round";
				beginPath();
					arc(node_array[i][0], node_array[i][1], node_array[i][2]-2.0, -0.5, Math.PI * 0.7, false);
					stroke();
				closePath();
