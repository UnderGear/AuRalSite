...
function splineSlider(canvas, ajax_command, slider_id) {
	...
	this.transmission = "ajax";			// [ajax, osc, max, aural]
	// if (navigator.userAgent == "AuRal") this.transmission = "aural";
	
	this.user_id = 2;
	this.url = "aural.allisonic.com";
	...
	this.aural_sender = aural_sender;
	...
	function init() {
		...
		if(is_touch_device) {
			...
			canvas.ontouchmove = self.throttle(slider_touch_move, 50);
	...
	function slider_change(click_location) {
		if (((1.0 - self.slider_value)*canvas_height).toFixed(0) != click_location.y) {
			...
			if (self.transmission == "aural") {
				window.aural_send.slider_value(self.slider_id, self.slider_value.toFixed(2));
			} else {
				self.aural_sender(self.ajax_command + "/" + self.user_id, "param_" + self.slider_id, self.slider_value.toFixed(2));
			}
			draw();
		}
	}
